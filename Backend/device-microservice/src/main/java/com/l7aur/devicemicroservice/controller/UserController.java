...
public class UserController {
    ...
    @PostMapping("/")
    @Operation(
            summary = "Save a new user in the database",
            description = "Creates a new user entry in the database",
            responses = {
                    @ApiResponse(responseCode = "201", description = "Successfully created a new user in the database"),
                    @ApiResponse(responseCode = "500", description = "Some error occurred while interrogating the database")
            }
    )
    ResponseEntity<@NonNull User> addUser(@RequestBody User user) {
    ...
    @Operation(
            ...
            responses = {
                    @ApiResponse(responseCode = "200", description = "Successfully removed the users from the database"),
                    @ApiResponse(responseCode = "500", description = "Some error occurred while interrogating the database")
            }
    )
    ResponseEntity<@NonNull UserDeleteReply> removeUser(@RequestBody UserDeleteRequest request) {
