...
import com.l7aur.authenticationmicroservice.model.Role;
...
public class AuthenticationService {
    ...
    public ResponseEntity<@NonNull Authentication> update(Authentication authentication) {
        try{
            var oldAuth =  authenticationRepository.findAuthenticationByUsername(authentication.getUsername());
            if(oldAuth.isEmpty()){
                return new ResponseEntity<>(authentication, HttpStatus.NOT_FOUND);
            }
            var newAuth = oldAuth.get();
            newAuth.setUsername(authentication.getUsername());
            if(!authentication.getPassword().equals(newAuth.getPassword()))
                newAuth.setPassword(passwordEncoder.encode(authentication.getPassword()));
            newAuth.setRole(authentication.getRole());
            return new ResponseEntity<>(authenticationRepository.save(newAuth), HttpStatus.OK);
        }
        catch (Exception e){
